...
app.post("/createRoom", async (req, res) => {
    ...
    try{
        ...
        const roomId = (await client.query(`INSERT INTO rooms(createdat, expireat, passwordhash, wsconnection) VALUES($1,$2,$3,$4) RETURNING id;`,[currentdate, tomorrow, passwordHash, wsConnectionKey])).rows[0].id
...
app.post("/room/:id/connect", async (req, res) => {
    ...
    const password = req.body.password
    try{
        let rightRoomPasswordHash = await client.query(`SELECT passwordhash FROM rooms WHERE id = $1;`, [roomId])
        rightRoomPasswordHash = rightRoomPasswordHash.rows[0].passwordhash
